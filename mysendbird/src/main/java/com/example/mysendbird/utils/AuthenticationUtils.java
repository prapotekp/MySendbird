...
import com.example.mysendbird.R;
...
public class AuthenticationUtils {
    ...
    public static void authenticate(Context context, String userId, String accessToken, AuthenticateHandler handler) {
        if (userId == null) {
            Log.i("[Module]", "[AuthenticationUtils] authenticate() => Failed (userId == null)");
            if (handler != null) {
        ...
        deauthenticate(context, isSuccess -> {
            PushUtils.getPushToken(context, (pushToken, e) -> {
                if (e != null) {
                    Log.i("[Module]", "[AuthenticationUtils] authenticate() => Failed (e: " + e.getMessage() + ")");
                    if (handler != null) {
                        handler.onResult(false);
                    }
                    return;
                ...
                Log.i("[Module]", "[AuthenticationUtils] authenticate(userId: " + userId + ")");
                SendBirdCall.authenticate(new AuthenticateParams(userId).setAccessToken(accessToken), (user, e1) -> {
                    if (e1 != null) {
                        Log.i("[Module]", "[AuthenticationUtils] authenticate() => Failed (e1: " + e1.getMessage() + ")");
                    ...
                    Log.i("[Module]", "[AuthenticationUtils] authenticate() => registerPushToken(pushToken: " + pushToken + ")");
                    SendBirdCall.registerPushToken(pushToken, false, e2 -> {
                        if (e2 != null) {
                            Log.i("[Module]", "[AuthenticationUtils] authenticate() => registerPushToken() => Failed (e2: " + e2.getMessage() + ")");
                        ...
                        Log.i("[Module]", "[AuthenticationUtils] authenticate() => OK");
                        if (handler != null) {
...
//    }
    ...
    public static void deauthenticate(Context context, DeauthenticateHandler handler) {
        ...
        Log.i("[Module]", "[AuthenticationUtils] deauthenticate(userId: " + SendBirdCall.getCurrentUser().getUserId() + ")");
        String pushToken = PrefUtils.getPushToken(context);
        if (!TextUtils.isEmpty(pushToken)) {
            Log.i("[Module]", "[AuthenticationUtils] deauthenticate() => unregisterPushToken(pushToken: " + pushToken + ")");
            SendBirdCall.unregisterPushToken(pushToken, e -> {
                if (e != null) {
                    Log.i("[Module]", "[AuthenticationUtils] unregisterPushToken() => Failed (e: " + e.getMessage() + ")");
    ...
    private static void doDeauthenticate(Context context, DeauthenticateHandler handler) {
        SendBirdCall.deauthenticate(e -> {
            if (e != null) {
                Log.i("[Module]", "[AuthenticationUtils] deauthenticate() => Failed (e: " + e.getMessage() + ")");
                showToastErrorMessage(context, e);
            } else {
                Log.i("[Module]", "[AuthenticationUtils] deauthenticate() => OK");
            }
    ...
    public static void autoAuthenticate(Context context, AutoAuthenticateHandler handler) {
        Log.i("[Module]", "[AuthenticationUtils] autoAuthenticate()");

        if (SendBirdCall.getCurrentUser() != null) {
            Log.i("[Module]", "[AuthenticationUtils] autoAuthenticate(userId: " + SendBirdCall.getCurrentUser().getUserId() + ") => OK (SendBirdCall.getCurrentUser() != null)");
            if (handler != null) {
        ...
        if (!TextUtils.isEmpty(userId) && !TextUtils.isEmpty(pushToken)) {
            Log.i("[Module]", "[AuthenticationUtils] autoAuthenticate() => authenticate(userId: " + userId + ")");
            SendBirdCall.authenticate(new AuthenticateParams(userId).setAccessToken(accessToken), (user, e) -> {
                if (e != null) {
                    Log.i("[Module]", "[AuthenticationUtils] autoAuthenticate() => authenticate() => Failed (e: " + e.getMessage() + ")");
                    showToastErrorMessage(context, e);
                ...
                Log.i("[Module]", "[AuthenticationUtils] autoAuthenticate() => registerPushToken(pushToken: " + pushToken + ")");
                SendBirdCall.registerPushToken(pushToken, false, e1 -> {
                    if (e1 != null) {
                        Log.i("[Module]", "[AuthenticationUtils] autoAuthenticate() => registerPushToken() => Failed (e1: " + e1.getMessage() + ")");
                        showToastErrorMessage(context, e1);
                    ...
                    Log.i("[Module]", "[AuthenticationUtils] autoAuthenticate() => authenticate() => OK (Authenticated)");
                    if (handler != null) {
            ...
            Log.i("[Module]", "[AuthenticationUtils] autoAuthenticate() => Failed (No userId and pushToken)");
            if (handler != null) {
                handler.onResult(null);
            }
        }
